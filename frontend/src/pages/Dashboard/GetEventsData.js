...
const formattedDetailEvent = (event, onCloseHandler) => {
  return {
    ...
    showBorrar: true, // Ojo: Ajusta esto según tu lógica de permisos (solo creador debería borrar)
  };
};
...
export const getRegisteredEvents = async (closeModalHandler) => {
  try {
    ...
    const formattedFromApi = events.data.map((item, index) => ({
      ...
      // Pasamos el detalle formateado
      formattedDetailEvent: formattedDetailEvent(item.event, closeModalHandler)
    }));
